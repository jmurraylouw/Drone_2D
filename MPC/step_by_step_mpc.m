...
t = xmpc_resamp.Time';
N = size(x_data,2); % Number of data samples
...
for k = 1:N % every timestep k
    ...
    if mod(k,0.03/Ts_mpc) == 0
        plot(info.Topt + t(k), info.Yopt(:,y_rows))
        ylim([-15, 10])
        hold on;
        plot(info.Topt + t(k), ref_data(y_rows,(0:ph)+k)')
        plot(info.Topt + t(k), info.Uopt, ':', 'LineWidth', 1)
        plot(info.Topt + t(k), mo_data(y_rows,(0:ph)+k)', '--', 'LineWidth', 1)
        hold off;
        pause
    end
end
